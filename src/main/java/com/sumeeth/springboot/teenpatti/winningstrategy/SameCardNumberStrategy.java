...
public class SameCardNumberStrategy extends AbstractWinningStrategy implements WinningStrategy {
    ...
    public Optional<Winner> execute(List<Player> assignedPlayers) {
        ...
        if (null != elligibleSameCardWinners && elligibleSameCardWinners.size() == 1) {
            return Optional.of(new Winner(Optional.ofNullable(elligibleSameCardWinners.get(0)), Winner.WIN_TYPE.SAME_CARD_NO));
        } else if (null != elligibleSameCardWinners && elligibleSameCardWinners.size() > 1) {
            return Optional.of(new Winner(elligibleSameCardWinners.stream()
